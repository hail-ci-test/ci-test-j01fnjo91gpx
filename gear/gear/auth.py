...
class Authenticator(abc.ABC):
    ...
    def authenticated_developers_only(self, redirect=True):
        return self._authenticated_users_with_condition(lambda u: u['is_developer'] == 1, redirect)

    def authenticated_developers_and_service(self, service: str, redirect=True):
        return self._authenticated_users_with_condition(
            lambda u: u['is_developer'] == 1 or u['username'] == service, redirect
        )

    def _authenticated_users_with_condition(self, predicate: Callable[[UserData], bool], redirect: bool):
        def wrap(fun: AuthenticatedAIOHTTPHandler):
            @self.authenticated_users_only(redirect)
            @wraps(fun)
            async def wrapped(request: web.Request, userdata: UserData, *args, **kwargs):
                if predicate(userdata):
